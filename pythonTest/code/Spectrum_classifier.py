...
n_target = 5
n_fold = 20
X = np.load("train_data.npy").astype(np.float32)
Y = np.load("train_label.npy").astype(np.float32)
...
class CNN_classifier(nn.Module):
    def __init__(self, bn=False):
        ...
        self.out = nn.Sequential(
            nn.Linear(2048, 1671),        # fully connected layer, output miner classes
...
train_loss = np.zeros(n_target * n_fold)
valid_loss = np.zeros(n_target * n_fold)
...
for t in range(n_target):
    ...
    for f in range(n_fold):
        print("Target %d Fold %d --------------------------------------------" % (t, f))
        train_data, test_data, train_label, test_label = train_test_split(X, YY, test_size=0.05)
        ...
        for epoch in range(EPOCH):
            for step, (x, y) in enumerate(train_loader):  # gives batch data, normalize x when iterate train_loader
                ...
                if step == 15:
                    ...
                    mse = loss_func(test_output, test_y)
                    print('Epoch: ', epoch,
                          '| Train loss: %.4f' % loss.data[0],
                          '| Test loss: %.4f' % mse.data.numpy()[0])
